    ...
    $.widget("jHtree.jHTree", {
            ...
            if (node.crosscat == true) {
                ...
                if (node.jenis == 'csf') {
                    ...
                    nodeLiElements += '<li id="'+node.id+'" class="tnode">';
                        nodeLiElements += beforeDiv;
                        nodeLiElements += '<div class="trcont">';
                            nodeLiElements += '<div class="ui-widget-header ui-corner-tl ui-corner-tr" data-id-cascade="'+node.id+'">';
                                nodeLiElements += node.head;
                            nodeLiElements += '</div>';
                            nodeLiElements += '<div class="ui-widget-content ui-corner-bl ui-corner-br">';
                                nodeLiElements += node.contents;
                    ...
                    var nodeLiElements = '';
                    if (node.level == 'immediate_outcome_1') {
                        if (node.has_ime == 0) {
                            nodeLiElements += '<li id="'+node.id+'" class="tnode" style="padding-right: 40px !important;">';
                        } else {
                            nodeLiElements += '<li id="'+node.id+'" class="tnode">';
                        }
                    } else if (node.level == 'immediate_outcome_2') {
                        nodeLiElements += '<li id="'+node.id+'" class="tnode" style="padding-right: 40px !important;">';
                    } else {
                        nodeLiElements += '<li id="'+node.id+'" class="tnode">';
                    }
                        nodeLiElements += beforeDiv;
                        nodeLiElements += '<div class="trcont">';
                            if (node.level == 'ultimate_outcome') {
                                nodeLiElements += '<div class="ui-widget-header ui-corner-tl ui-corner-tr" style="background: red !important;">';
                            } else if (node.level == 'intermediate_outcome_1') {
                                nodeLiElements += '<div class="ui-widget-header ui-corner-tl ui-corner-tr" style="background: blue !important;">';
                            } else if (node.level == 'intermediate_outcome_2') {
                                nodeLiElements += '<div class="ui-widget-header ui-corner-tl ui-corner-tr" style="background: #1963d5 !important;">';
                            } else if (node.level == 'immediate_outcome_1') {
                                nodeLiElements += '<div class="ui-widget-header ui-corner-tl ui-corner-tr" style="background: green !important;">';
                            } else if (node.level == 'immediate_outcome_2') {
                                nodeLiElements += '<div class="ui-widget-header ui-corner-tl ui-corner-tr" style="background: #19d554 !important;">';
                            } else if (node.level == 'output') {
                                nodeLiElements += '<div class="ui-widget-header ui-corner-tl ui-corner-tr" style="background: orange !important;">';
                            } else {
                                nodeLiElements += '<div class="ui-widget-header ui-corner-tl ui-corner-tr">';
                            }
                            if (node.inovasi == 1) {
                                nodeLiElements += '<span class="inovasi-icon"><i class="fa fa-lightbulb-o"></i></span><br><span class="inovasi-judul">';
                            }
                                nodeLiElements += node.head;
                            nodeLiElements += '</span></div>';
                            let border_radius = '';
                            if(typeof(node.list_unit) != "undefined" && node.list_unit !== null || typeof(node.list_unit) != "undefined" && node.list_unit !== null) {
                            //     bla();
                            // }
                            // if (node.list_unit.length > 0) {
                                border_radius = 'border-radius-none';
                            ...
                            nodeLiElements += '</div>';
                            if(typeof(node.list_unit) != "undefined" && node.list_unit !== null) {
                            //     bla();
                            // }
                            // if (node.list_unit.length > 0) {
                                nodeLiElements += '<div class="ui-widget-content-unit ui-corner-bl ui-corner-br">';
                            ...
                            if(typeof(node.unor) != "undefined" && node.unor !== null) {
                                //     bla();
                                // }
                                // if (node.list_unit.length > 0) {
                                    nodeLiElements += '<div class="ui-widget-content-unit ui-corner-bl ui-corner-br">';
                                    nodeLiElements += node.unor.name;
                                    nodeLiElements += '</div>';
                                }
                        nodeLiElements += '</div>';
                        nodeLiElements += '<div class="trchl">';
                        if (node.level !== 'Output') {
                            if (node.level == 'immediate_outcome_1') {
                                if (node.has_ime == 0) {
                                    nodeLiElements += '<ul class="last">'+bfrul+'</ul>';
                                } else {
                                    nodeLiElements += '<ul>'+bfrul+'</ul>';
                                }
                            } else if (node.level == 'immediate_outcome_2') {
        ...
        _interactionEvents: function () {
            ...
            */
            ...
            $(".tree").on("click", ".trcont > .ui-widget-content", function () {
                var id_cascade = $(this).parent().find('.ui-widget-header:first').data('id-cascade');
                show_info(id_cascade);
            });
